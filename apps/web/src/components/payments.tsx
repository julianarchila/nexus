...
import type { paymentProcessors, countryProcessorFeatures } from "@/core/db/schema";
import {
    Table,
    TableBody,
    TableCell,
...
export function ProcessorsTable({ initialData, initialPagination }: ProcessorsTableProps) {
        ...
        <div className="space-y-4">
                    ...
                    <TableBody>
                        {processors.length === 0 ? (
                            ...
                            processors.map((processor) => (
                                <TableRow key={processor.id} className="hover:bg-gray-50 last:border-0">
                                    <TableCell className="font-medium">{processor.name}</TableCell>
                                    <TableCell className="text-muted-foreground font-mono text-xs">{processor.id}</TableCell>
                                    <TableCell>{getStatusBadge(processor.status)}</TableCell>
                                    <TableCell className="text-center">
                                        <span className={processor.supports_payouts ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}>
                                            {processor.supports_payouts ? "✓" : "—"}
                                        </span>
                                    </TableCell>
                                    <TableCell className="text-center">
                                        <span className={processor.supports_recurring ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}>
                                            {processor.supports_recurring ? "✓" : "—"}
                                        </span>
                                    </TableCell>
                                    <TableCell className="text-center">
                                        <span className={processor.supports_refunds ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}>
                                            {processor.supports_refunds ? "✓" : "—"}
                                        </span>
                                    </TableCell>
                                    <TableCell className="text-center">
                                        <span className={processor.supports_crypto ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}>
                                            {processor.supports_crypto ? "✓" : "—"}
                                        </span>
                                    </TableCell>
                                    <TableCell className="text-muted-foreground text-sm">{processor.product_manager || "—"}</TableCell>
                                </TableRow>
                            ))
                        )}
                    </TableBody>
            ...
            <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">
                    Showing <span className="font-medium text-foreground">{processors.length}</span> of{" "}
                    <span className="font-medium text-foreground">{pagination.total}</span> processors
                </p>
...
export function CountryFeaturesTable({ initialData, initialPagination }: CountryFeaturesTableProps) {
    ...
    return (
        <div className="space-y-4">
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="hover:bg-transparent">
                            ...
                            <TableHead className="font-semibold">Status</TableHead>
                            <TableHead className="font-semibold text-center">Local Inst.</TableHead>
                            <TableHead className="font-semibold text-center">Payouts</TableHead>
                            <TableHead className="font-semibold text-center">Crypto</TableHead>
                    ...
                    <TableBody>
                        {features.length === 0 ? (
                            ...
                            features.map((feature) => (
                                <TableRow key={feature.id} className="hover:bg-gray-50 last:border-0">
                                    <TableCell className="font-medium">{feature.country}</TableCell>
                                    <TableCell className="text-muted-foreground font-mono text-xs">{feature.processor_id}</TableCell>
                                    <TableCell>{getStatusBadge(feature.status)}</TableCell>
                                    <TableCell className="text-center">
                                        <span className={feature.supports_local_instruments ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}>
                                            {feature.supports_local_instruments ? "✓" : "—"}
                                        </span>
                                    </TableCell>
                                    <TableCell className="text-center">
                                        <span className={feature.supports_payouts ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}>
                                            {feature.supports_payouts ? "✓" : "—"}
                                        </span>
                                    </TableCell>
                                    <TableCell className="text-center">
                                        <span className={feature.supports_crypto ? "text-green-600 dark:text-green-400" : "text-muted-foreground"}>
                                            {feature.supports_crypto ? "✓" : "—"}
                                        </span>
                                    </TableCell>
                                    <TableCell>
                                        <div className="flex flex-wrap gap-1.5">
                                            {feature.supported_methods && feature.supported_methods.length > 0 ? (
                                                <>
                                                    {feature.supported_methods.slice(0, 3).map((method, idx) => (
            ...
            <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">
                    Showing <span className="font-medium text-foreground">{features.length}</span> of{" "}
                    <span className="font-medium text-foreground">{pagination.total}</span> features
                </p>
                <div className="flex items-center gap-2">
                    <Button
                        variant="outline"
